  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center overflow-auto py-8' style={{ backgroundImage: `url(${bg})` }} >
      <form className='w-[90%] min-h-[500px] py-[40px] max-w-[450px] bg-black/40 backdrop-blur-xl shadow-2xl shadow-black/50 flex flex-col items-center justify-center gap-[20px] px-[20px] rounded-3xl border border-white/10' onSubmit={handleSignUp}>
        ...
        <input type="text" placeholder='Enter your Name' className='w-full h-[60px] outline-none border-2 border-white bg-transparent  text-white placeholder-gray-300 rounded-full text-[18px]' style={{ paddingLeft: '40px', paddingRight: '40px' }} required onChange={(e) => setName(e.target.value)} value={name} />
        <input type="email" placeholder='Email' className='w-full h-[60px] outline-none border-2 border-white bg-transparent  text-white placeholder-gray-300 rounded-full text-[18px]' style={{ paddingLeft: '40px', paddingRight: '40px' }} required onChange={(e) => setEmail(e.target.value)} value={email} />
        <div className='w-full h-[60px] border-2 border-white bg-transparent  text-white rounded-full text-[18px] relative'>
          <input type={showPassword ? "text" : "password"} placeholder='password' className='w-full h-full rounded-full outline-none bg-transparent placeholder-gray-300' style={{ paddingLeft: '40px', paddingRight: '40px' }} required onChange={(e) => setPassword(e.target.value)} value={password} />
